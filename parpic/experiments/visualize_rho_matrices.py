...
def plot_matrices(P: np.ndarray, P_nu: np.ndarray, P_sym: np.ndarray, output_path: Path, show: bool) -> None:
...
def main() -> None:
  parser = argparse.ArgumentParser(description="Visualize rho-dependent matrix operators for directed SBM")
  parser.add_argument("--model", type=str, default="chain", choices=["chain", "cp"], help="SBM model type")
  parser.add_argument("--rho", type=float, default=0.3, help="Inter-cluster coupling parameter")
  parser.add_argument("--block_size", type=int, default=130, help="Size of each of the 3 blocks")
  parser.add_argument("--gamma", type=float, default=0.5, help="Gamma parameter for sum_deg")
