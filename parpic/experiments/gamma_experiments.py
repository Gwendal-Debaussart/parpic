...
def run_gamma_trial(gamma_val, A, y, P, t, run_id, seed=42):
    ...
    k_means = KMeans(
        n_clusters=len(np.unique(y)), n_init=100, random_state=seed + run_id
    ).fit(L_emb)
...
if __name__ == "__main__":
    ...
    t = int(
        get_time_iteration(
            parametrized_laplacian(P, sum_deg(A, gamma=0.5), normalized=True), t_max=250
        )
    )
    ...
    print(
        f"Running {len(gamma_range)} gamma values with {n_runs} trials each in parallel..."
    )
    ...
    for gamma_val in gamma_range:
        ...
        print(
            f"gamma: {gamma_val:.2f}, AMI: {ami_scores[gamma_val]:.4f} ± {ami_stds[gamma_val]:.4f}"
        )
        idx += n_runs
